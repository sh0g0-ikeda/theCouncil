...
- force_tradeoff/refocusのみ: {"content":"...","stance":"facilitate","main_axis":"...","constraint":"次の2レスは〜のみ","constraint_turns":2,"constraint_kind":"tradeoff|refocus"}"""
...
async def make_facilitate(
    ...
    user_content = (
        ...
        f"未解決の定義語: {', '.join(unresolved_terms[:3]) or 'なし'}\n"
        f"未回答claim数: {open_claim_count}\n"
        f"{agent_info}"
        f"【機能】{fn_instruction}\n"
        f"最近の発言:\n{transcript}"
    )
...
def _select_facilitator_function(posts: list[dict[str, Any]], debate: Any = None) -> str:
    ...
    if open_claim_count >= 4:
        return "refocus"
    if open_claim_count >= 2 and has_contested:
        return "force_tradeoff"
