...
- concretize: 具体的な歴史事例・制度名・数字を1つ挙げ、「この例が証明するのは〜」という役割を明示して論じる（例「スハルト体制の高成長期が証明するのは〜だが、1997年危機が示すのは〜」）
...
_PHASE_DIRECTIVES: dict[int, str] = {
    1: "【フェーズ1・定義期】まずテーマの核心語を自分の思想から定義せよ（例：「一党独裁」とは何か、「成長」とは何を指すか、「長期的」とは何年か）。定義なき議論は全員が別の話をする。定義後、その定義に基づく自分の結論を一言で断言せよ。",
    2: "【フェーズ2・対立期】相手の主張の前提（何を「成長」と定義しているか、体制のおかげか外部環境か）を突け。事例を使う場合は「この例が証明するのは〜」という役割を明示せよ。",
    3: "【フェーズ3・激化期】最も鋭い攻撃か steelman→崩しを出せ。歴史の具体例で相手の論を実証的に崩せ。",
    4: "【フェーズ4・転換期】これまでの論点より一段深い角度か、相手が見落としている歴史的事実・データを突きつけよ。",
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "議題に対して「この問いをどの基準で判断するか」という争点軸を4〜6個生成せよ。"
                        "軸は議題の内容に即した具体的なものにせよ。"
                        "法的問いなら「国際法上の合法性」「主権侵害の有無」、"
                        "政策問いなら「安全保障上の費用対効果」「エスカレーションリスク」「歴史的先例の適用可能性」、"
                        "倫理問いなら「正戦論的許容性（比例原則・区別原則）」「結果主義的合理性」等。"
                        "「効率性」「公平性」「多様性」のような汎用抽象語は禁止。この議題にしか当てはまらない軸を出せ。"
                        "短い日本語の名詞句で。"
                        '形式: {"axes": ["軸1", "軸2", ...]}'
                    ),
                },
