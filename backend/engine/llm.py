...
async def generate_debate_script(
    ...
    system_msg = (
        ...
        "\n"
        "【各sideの行動原則（必ず守れ）】\n"
        "  support: 議題を肯定する立場。自分のworldviewを根拠に「なぜ支持するか」を攻める。\n"
        "  oppose: 議題の前提を否定する立場。「そもそもその前提が間違っている」と根本から崩す。\n"
        "  neutral: 賛否どちらの陣営にも乗らず、自分のworldview固有の評価軸で両陣営を批判・再定義する。\n"
        "    ✗ neutralの失格パターン: oppose陣営と同じ結論・同じ論拠に流れる（最も多い失敗）\n"
        "    ✓ neutralの正解パターン: 「賛否の対立軸そのものが自分の価値観から見て間違っている」と第三の視点を立てる\n"
        "    neutralのdirectiveには必ず「どちらの陣営も触れていない評価軸」を指定せよ。\n"
        "\n"
        "【絶対条件1：真の対立】\n"
